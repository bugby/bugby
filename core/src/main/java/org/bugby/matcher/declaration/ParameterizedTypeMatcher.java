...
public class ParameterizedTypeMatcher extends DefaultTreeMatcher implements TreeMatcher {
	private final TypeMirror patternType;
	private final TreeMatcher typeDefMatcher;
	private final TreeMatcherFactory factory;

	public ParameterizedTypeMatcher(ParameterizedTypeTree patternNode, TreeMatcherFactory factory) {
		super(patternNode);
		Element element = TreeUtils.elementFromUse(patternNode.getType());
		patternType = element.asType();
		typeDefMatcher = factory.buildForType(patternType.toString());
		this.factory = factory;
		//TODO check for type arguments
	}
