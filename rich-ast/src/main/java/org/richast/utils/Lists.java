...
public final class Lists {
	...
	public static <T> T getOnlyElement(Iterable<T> elems) {
		if (elems instanceof List<?>) {
			...
			PreConditions.checkState(list.size() == 1, "elems must contain exactly one element");
			return list.get(0);
		}
		Iterator<T> iterator = elems.iterator();
		PreConditions.checkState(iterator.hasNext(), "elems must not be empty");
		T obj = iterator.next();
		PreConditions.checkState(!iterator.hasNext(), "elems must contain exactly one element");
		return obj;
	}
