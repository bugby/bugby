...
public class SomeVariableMatcherFactory extends DefaultMatcherFactory {
	...
	protected WildcardNodeMatcher buildPatternNodeOnly(TreeModel<Node, Node> patternSourceTreeNodeModel, Node currentPatternSourceNode,
		...
		if (var != null) {
			...
			return new SomeVariableMatcher(var.getName(), scope);
		}
		throw new RuntimeException("SomeVariableMatcher used, but no variable is defined on node:" + currentPatternSourceNode);
	}
